...
def main():
    ...
    seed = 1024
    np.random.seed(seed)
    path = config.FEAT_PATH

    #re load to avoid errors. 
    
    train = pd.read_csv(config.RAW_PATH+"train.csv", nrows=config.TRAIN_SIZE)
    test = pd.read_csv(config.RAW_PATH+"test.csv", nrows=config.TEST_SIZE)
